<div>
    <header>
        <div class="px-4 mx-auto mb-5 max-w-7xl sm:px-6 lg:px-8">
            <h1 class="text-3xl font-bold leading-tight text-gray-900">Update Task</h1>
        </div>
    </header>
            ...
            <form action="#">
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <div>
                        ...
                        <select id="statusReference" wire:model='status'
                            class="block w-full p-2 text-sm text-gray-900 border border-gray-300 rounded-lg px- bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    ...
                    @isset($task->upload)
                    @if($task->upload->count() > 0)
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Attachment</label>
                    <div class="flex justify-center w-full">
                        <img class="h-auto max-w-lg rounded-lg" src="{{ Storage::url($task->upload->first()->path) }}"
                    ...
                    <div class="px-6 py-4 border rounded">
                        <div class="w-full">
                            ...
                            <div>
                                <label for="statusReference"
                                    class="block text-sm font-medium text-gray-900 dark:text-white whitespace-nowrap">Status</label>
                                <select id="statusReference" wire:model='subTasks.{{$idx}}.status'
                                    class="block w-full p-2 text-sm text-gray-900 border border-gray-300 rounded-lg px- bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    @foreach ($statusReferences as $key => $statusReference)
                                    <option value="{{$key}}">{{$statusReference}}</option>
                                    @endforeach
                                </select>
                                @error('subTasks.{{$idx}}.status')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span
                                        class="font-medium">{{$message}}
                                </p>
                                @enderror
